...
so one can use the same tools and techniques for analyzing simulated traffic as used on real traffic. Knowledge of pcap can be reused in the context of simulations.
...
The module records packets containing IPv4 packets (packets not containing IPv4 packets are discarded).
